...
export const useCareerStore = create<CareerState>((set) => ({
  ...
  fetchCareers: async () => {
    ...
    try {
      const res = await http.get("/api/careers")
      const data = res.data
  ...
  createCareer: async (payload) => {
    try {
      const res = await http.post("/api/careers", payload)
  ...
  updateCareer: async (id, payload) => {
    try {
      const res = await http.put(`/api/careers/${id}`, payload)
  ...
  deleteCareer: async (id) => {
    try {
      await http.delete(`/api/careers/${id}`)
