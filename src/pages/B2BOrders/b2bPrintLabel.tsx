...
/**
 * Generates a 4x6 shipping label for B2B orders using the b2b invoice
 * endpoint's data shape (billTo, shipTo, driver, vehicle, shipment, charges)
 * instead of the retail invoice / issuedByAgency structure.
 *
 * Same visual layout as handleLabel — barcode strips are plain text —
 * but the right-hand data panes are re-mapped to what the b2b payload
 * actually contains (no issuedByAgency, no parcelDetails, driver + fuller
 * vehicle info instead).
 */
export const handleB2BLabel = async (orderId: string): Promise<void> => {
    try {
        ...
        const amount = charges?.totalAmount ?? 0
        const transportationCharge = charges?.transportationCharge ?? 0

        const orderDate = invoiceData?.invoiceDate || invoiceData?.createdAt || ""
        const dObj = orderDate ? new Date(orderDate) : new Date()
        const dateStr = dObj.toLocaleDateString()

        const pickupAddress = invoiceData?.pickupAddress || billTo?.address || ""
        const deliveryAddressFull = invoiceData?.deliveryAddress || shipTo?.address || ""

        const trackingNo = String(
            invoiceData?.lrNo || invoiceData?.orderNumber || invoiceData?.invoiceNumber || orderId
        )
        const waybill = invoiceData?.waybill || ""
        const vehicleType = vehicle?.vehicleType || ""
        const vehicleReg = vehicle?.registrationNumber || ""
        const vehicleCapacity = vehicle?.capacityKg || ""
        const weight = shipment?.approximateWeight

        // B2B payload has no agency/franchise block — fall back to a fixed brand name.
        const franchiseName = "TrueCargo"

        const returnAdd = [billTo?.name, billTo?.address, billTo?.state, billTo?.pincode]
            .filter(Boolean).join(", ")

        // -------------------------------------------------------------------
        // Build the 4x6 label (same visual layout as the retail label)
        // -------------------------------------------------------------------
        const doc = new jsPDF({
        ...
        // Row 2: Tracking Number strip (Y: 0.6 -> 1.5)
        ...
        // b2b shipTo has no city field, so leave the right routing slot blank/pincode-based
        doc.setFont("helvetica", "bold");
        doc.text(String(shipTo?.pincode || ""), 3.85, 1.45, { align: "right" });
        ...
        // -- Waybill / Vehicle / Driver (b2b has richer vehicle + driver data) --
        let nextY = 2.80;
        if (waybill) {
            nextY += 0.15;
            doc.setFontSize(8);
            ...
            nextY += (waybillLines.length - 1) * 0.12;
        }
        if (vehicleType || vehicleReg) {
            ...
            doc.setFont("helvetica", "normal");
            const vehicleLine = [vehicleType].filter(Boolean).join(" / ");
            const vehicleLines = doc.splitTextToSize(vehicleLine, 3.9 - (2.85 + vehicleLabelW));
            doc.text(vehicleLines, 2.85 + vehicleLabelW, nextY);
            nextY += (vehicleLines.length - 1) * 0.12;
        }
        // if (vehicleCapacity) {
        //     nextY += 0.13;
        //     doc.setFont("helvetica", "bold");
        //     doc.text("Capacity: ", 2.85, nextY);
        //     doc.setFont("helvetica", "normal");
        //     doc.text(`${vehicleCapacity} kg`, 2.85 + doc.getTextWidth("Capacity: "), nextY);
        // }
        // if (driver?.driverName) {
        //     nextY += 0.13;
        //     doc.setFont("helvetica", "bold");
        //     doc.text("Driver: ", 2.85, nextY);
        //     doc.setFont("helvetica", "normal");
        //     const driverLabelW = doc.getTextWidth("Driver: ");
        //     const driverLine = [driver.driverName, driver.phoneNumber].filter(Boolean).join(" / ");
        //     const driverLines = doc.splitTextToSize(driverLine, 3.9 - (2.85 + driverLabelW));
        //     doc.text(driverLines, 2.85 + driverLabelW, nextY);
        // }
        ...
        // Row 6: Charges breakdown — b2b payload only has transportationCharge
        ...
        // Row 8: Bottom tracking number strip (Y: 4.4 -> 5.15)
        doc.line(0.1, 5.15, 3.9, 5.15);
        doc.setFont("helvetica", "bold");
        doc.setFontSize(16);
        doc.text(trackingNo, 2.0, 4.90, { align: "center" });

        // Row 9: Return address + GST (Y: 5.15 -> 5.9)
        doc.setFont("helvetica", "normal");
        doc.setFontSize(9);
        let footerY = 5.30;
        ...
        if (billTo?.gstNumber) {
            doc.setFont("helvetica", "bold");
            doc.text(`GSTIN: ${billTo.gstNumber}`, 0.15, footerY + 0.05);
        }
