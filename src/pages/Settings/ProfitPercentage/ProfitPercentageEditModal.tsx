...
import { useProfitPercentageStore, type ProfitPercentageItem } from "../../../store/profitPercentageStore"

interface Props {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
  item: ProfitPercentageItem | null
}
...
const ProfitPercentageEditModal: FC<Props> = ({ isOpen, onClose, onSuccess, item }) => {
  const { updatePercentages, loading } = useProfitPercentageStore()
      ...
      <div className="relative max-h-[90vh] overflow-y-auto rounded-lg bg-white shadow-2xl dark:bg-gray-800 [scrollbar-width:thin] [scrollbar-color:#64748b_transparent] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-500/70 hover:[&::-webkit-scrollbar-thumb]:bg-slate-400/80">
        ...
        <Formik
          ...
          initialValues={{
            ...
            loadingChargePercentage: String(item.loadingChargePercentage ?? ""),
            miscChargePercentage: String(item.miscChargePercentage ?? ""),
          }}
          validationSchema={validationSchema}
          onSubmit={async (values, { setSubmitting }) => {
            try {
              await updatePercentages(item.id, {
          ...
          {({ values, handleChange, handleBlur, touched, errors, isSubmitting }) => (
            <Form className="p-6">
              <FormSection
                ...
                description={`Update charges for ${item.agencyName || "selected agency"}`}
                icon={<HiOfficeBuilding className="h-5 w-5" />}
              >
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div className="md:col-span-2">
                    <Label value="Agency" />
                    <TextInput className="mt-2" value={item.agencyName || "-"} readOnly />
                  </div>
              ...
              <div className="flex gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                ...
                <Button type="submit" disabled={isSubmitting || loading} className="flex-1 bg-orange-500 hover:bg-orange-600">
                  Update
                </Button>
              </div>
            </Form>
...
export default ProfitPercentageEditModal
