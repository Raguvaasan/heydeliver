...
const FranchiseStaffDetailPage = () => {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const { getStaffById, selectedStaff, loading, error } = useFranchiseStaffStore()
  const [activeTab, setActiveTab] = useState("franchise")

  useEffect(() => {
    if (id) getStaffById(id)
  }, [id, getStaffById])

  const handleEdit = () => {
    // Navigate back to franchise staff list and open edit modal
    navigate("/franchise-staff", { state: { editStaffId: id } })
  }
          ...
          <Card>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-base font-medium text-gray-900 dark:text-white mt-1">
                  {(staff as any).mobile || (staff as any).phone}
                </p>
              </div>
              <div>
...
export default FranchiseStaffDetailPage
